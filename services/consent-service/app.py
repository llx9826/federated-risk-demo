...
async def login(request: LoginRequest):
    """用户登录"""
    # 简单的用户验证（生产环境应使用数据库和密码哈希）
    users = {
        "admin": {"password": "admin123", "role": "admin", "user_id": "admin_001"},
        "bank_user": {"password": "bank123", "role": "bank_user", "user_id": "bank_001"},
        "ecom_user": {"password": "ecom123", "role": "ecom_user", "user_id": "ecom_001"}
    }
...
@app.get("/federated/jobs")
async def get_federated_jobs():
    """获取联邦训练任务列表"""
    return {
        "jobs": [],
        "total": 0,
